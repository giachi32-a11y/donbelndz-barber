      ...
      <Routes>
        ...
        <Route path="/prenota" element={
          <div style={{width: '100%', maxWidth: '360px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '20px'}}>
            ...
            {oraSel && (
              <button onClick={() => {
                const msg = `Ciao Donblenz! Vorrei prenotare ${localStorage.getItem('serv')} il ${dataSel} alle ore ${oraSel}.`;
                window.open(`https://wa.me/393447875378?text=${encodeURIComponent(msg)}`);
              }} style={{...styles.mainButton, marginTop:'40px', width:'100%'}}>CONFERMA SU WHATSAPP</button>
            )}
          </div>
        } />
      </Routes>
